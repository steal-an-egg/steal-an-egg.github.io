...
以下步骤说明如何快速添加一个新游戏页面。请将 `[GAME_SLUG]` (例如 `pokemon-gamma-emerald`)、`[GAME_NAME]` (例如 "Pokemon Gamma Emerald") 等占位符替换为实际游戏信息。

### 1. 创建游戏页面文件

#### 1.1 复制现有游戏目录
1. 在 `app/` 目录下找到任意一个现有游戏目录（如 `app/pokemon-gamma-emerald/`）
2. 复制整个目录并重命名为 `app/[GAME_SLUG]/`

#### 1.2 修改页面文件 (`page.tsx`)
打开 `app/[GAME_SLUG]/page.tsx`，替换以下内容：
- 导入语句中的内容变量名（改为驼峰式命名，如 `pokemonGammaEmeraldContent`）
- `metadata` 中的 `title` 和 `description`
- `gameConfig.metadata` 中的相关信息
- 组件函数名（如 `PokemonGammaEmeraldPage`）

#### 1.3 修改内容配置文件 (`content.ts`)
打开 `app/[GAME_SLUG]/content.ts`，替换以下内容：
- 导出变量名（改为驼峰式命名，如 `pokemonGammaEmeraldContent`）
...
#### 5.1 复制现有游戏嵌入目录
1. 复制 `public/game/pokemon-gamma-emerald/` 目录
...
3. **命名规范**：
   - 文件夹和URL使用短横线分隔（如 `pokemon-gamma-emerald`）
   - 变量名使用驼峰式命名，可以适当简化（如 `pokemonGammaEmeraldContent` 或 `sprunkiIncredboxContent`）
   - 组件函数名使用PascalCase（如 `PokemonGammaEmeraldPage`）
